...
export function createThaliDescription(thali) {
  ...
  return `${name.toUpperCase()} (${vegNonVeg}) - Items: ${itemsStr} - Rs.${priceStr}`;
}
...
export function getThaliStats(thalis) {
  ...
  const prices = thalis.map(t => t.price).filter(p => typeof p === 'number');
...
export function searchThaliMenu(thalis, query) {
  ...
  return thalis.filter(thali => {
    if (typeof thali.name === 'string' && thali.name.toLowerCase().includes(lowerQuery)) {
      return true;
    }
    if (Array.isArray(thali.items)) {
      return thali.items.some(item => typeof item === 'string' && item.toLowerCase().includes(lowerQuery));
    }
    return false;
  });
}
