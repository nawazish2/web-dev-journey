...
export function iplPointsTable(matches) {
  ...
  for (const match of matches) {
    ...
    // Initialize teams in the accumulator if not already present
    if (!teams[team1]) {
  ...
  // Convert the teams object to an array and sort it
  const pointsTable = Object.values(teams);
  pointsTable.sort((a, b) => {
    if (b.points !== a.points) {
      return b.points - a.points; // Sort by points DESCENDING
    }
    return a.team.localeCompare(b.team); // If points are equal, sort by team name ASCENDING
  });
  return pointsTable;
}
