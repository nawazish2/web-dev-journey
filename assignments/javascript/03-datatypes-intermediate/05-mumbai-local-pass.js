...
export function generateLocalPass(passenger) {
  ...
  if (
    ...
    !to || typeof to !== "string" ||
    !classType || typeof classType !== "string"
  ) {
    return "INVALID PASS";
  }
  ...
  const classChar = lowerClass.charAt(0).toUpperCase();
  const fromPart = from.slice(0, 3).toUpperCase();
  const toPart = to.slice(0, 3).toUpperCase();
  const passId = `${classChar}${fromPart}${toPart}`;

  const titleCaseFrom = from.charAt(0).toUpperCase() + from.slice(1).toLowerCase();
  const titleCaseTo = to.charAt(0).toUpperCase() + to.slice(1).toLowerCase();
